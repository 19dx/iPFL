...
from .utils_language import letter_to_vec, word_to_indices
...
def split_leaf_data(args):
    ...
    if args.leaf_sample_top:
        ...
        rng.shuffle(indices)
    # delete the users with 0 samples
    indices = [i for i in indices if usrs_samples_num[i] > 0]
    ...
    test_indices = indices[args.leaf_train_num:args.leaf_train_num+args.leaf_test_num]  # obtain the test index
...
def leaf_read(args):
    ...
    if args.dataset == 'femnist':
        ...
        test_dl = DataLoader(dataset=test_ds, batch_size=64, shuffle=True)
    ...
    testdata_cls_counts = count_class(test_ds, num_classes, range(len(test_ds)))
    test_distribution = testdata_cls_counts / testdata_cls_counts.sum()
    for net_id in train_user_index: 
        ...
        train_dl_local = DataLoader(dataset=train_ds_local, batch_size=args.batch_size, shuffle=True, drop_last=False)
    ...
    return train_dataloaders, (test_dl, test_distribution), client_num_samples, traindata_cls_counts, data_distributions
